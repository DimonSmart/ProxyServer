...
- **Security**: HTTPS support with development certificates
- **Command Line Interface**: Management commands for cache operations

## 🚀 Quick Start

1. **Setup HTTPS development certificate (one-time):**
   ```bash
...
3. **Access your proxy:**
   ...
   - HTTPS: `https://localhost:8043` (auto-enabled with dev certificate)

> **💡 Pro Tip:** HTTPS is automatically enabled when you specify `HttpsPort` in settings.json.
...
```json
{
  ...
  "Port": 8042,
  "HttpsPort": 8043,          // ← Simply add this to enable HTTPS with dev certificate
  "MemoryCache": {
    "TtlSeconds": 1800,
    "MaxEntries": 10000
  },
...
The proxy server supports HTTPS using .NET development certificates. No manual certificate management is required for development.

#### Quick Start

1. **Install .NET development certificate (one-time setup):**
   ```bash
   dotnet dev-certs https --trust
   ```

2. **Configure HTTPS in settings.json:**
   ```json
   {
     "Port": 8042,        // HTTP port
     "HttpsPort": 8043    // HTTPS port (enables HTTPS automatically)
   }
   ```
...
The development certificate is automatically:
- Generated by .NET SDK
- Trusted by your system when using `--trust` flag
- Used by the proxy server when `HttpsPort` is specified
- Valid for localhost and loopback addresses
...
- Azure Key Vault or similar certificate management services

> **Note**: The development certificate approach is perfect for development, testing, and local deployments. For production, implement proper certificate management according to your infrastructure requirements.
